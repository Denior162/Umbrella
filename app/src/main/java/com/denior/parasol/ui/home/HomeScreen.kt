...
import com.denior.parasol.network.model.Forecast
import com.denior.parasol.network.model.UvResponse
import com.denior.parasol.ui.theme.LocalRiskExtendedColorScheme
import com.denior.parasol.ui.theme.getUVCardColors
import com.denior.parasol.utils.UvRiskLevel
...
fun HomeScreen(
    ...
    Scaffold(topBar = {
        HomeScreenTopAppBar(
            ...
            retryAction = retryAction, isLoadingCities = isLoadingCities
        )
    }) { innerPadding ->
        Column(
            modifier = Modifier
...
fun ForecastCard(
    ...
    val riskExtendedColorScheme = LocalRiskExtendedColorScheme.current
    val cardColor = getUVCardColors(forecastGroup.items.first().uvi, riskExtendedColorScheme)
        ...
        Column(modifier = Modifier.padding(12.dp)) {
            Row(
                ...
                Column(modifier = Modifier.weight(1F)) {
                    val forecastDate = parseDate(forecastGroup.items.first().time).toLocalDate()
                    val today = LocalDate.now()
                    val dateText = when {
                        forecastDate.isEqual(today) -> stringResource(R.string.today)
                        forecastDate.isEqual(today.plusDays(1)) -> stringResource(R.string.tomorrow)
                        else -> forecastDate.format(DateTimeFormatter.ofPattern("dd MMMM yyyy"))
                    }
                    Row {
                        Column {
...
fun TextLevelRisk(group: UvRiskLevel) {
    Text(
        text = when (group) {
            UvRiskLevel.LOW -> stringResource(R.string.low_risk)
            UvRiskLevel.MODERATE -> stringResource(R.string.moderate_risk)
            UvRiskLevel.HIGH -> stringResource(R.string.high_risk)
